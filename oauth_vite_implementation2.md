...
| OAuth callback page | Exchanges code for tokens, calls Frappe, redirects to dashboard |
| **Frappe token exchange** | **Sends IDP access_token to Frappe backend to get Frappe user profile** |
| Dashboard page | Shows IDP user info + Frappe user info, logout button |
| Auth context | Global auth state (user, frappeUser, token, loading, error) |
| Session persistence | OIDC tokens in `localStorage` via `oidc-client-ts` + Frappe user in `localStorage` |
...
/**
 * Frappe-side user record returned after exchanging the IDP access_token
 * with the Frappe backend (everydaybankingsolutions.facilitix.app).
 */
...
/** Map OIDC User profile to a clean AppUser, optionally enriched by Frappe data */
export function buildAppUser(oidcUser: User, frappe?: FrappeUser | null): AppUser {
  return {
    email:   frappe?.email    ?? oidcUser.profile.email   ?? '',
    name:    frappe?.full_name ?? oidcUser.profile.name   ?? oidcUser.profile.email ?? 'User',
    picture: frappe?.user_image ?? (oidcUser.profile.picture as string | undefined),
    sub:     oidcUser.profile.sub,
  };
}

// ── Frappe Token Exchange ──────────────────────────────────────────────────

/**
 * Exchange the IDP access_token with the Frappe backend.
 *
 * Strategy (two attempts, falls back gracefully):
 *
 * 1️⃣ POST to Frappe's social login callback for 'ebs' provider:
 *    /api/method/frappe.integrations.oauth2_logins.custom/ebs
 *    — Frappe validates the Bearer token via the configured OIDC provider
 *      and returns the Frappe user record.
 *
 * 2️⃣ Fallback: GET /api/method/frappe.auth.get_logged_user
 *    — Works if Frappe is already configured as an OIDC resource server
 *      and will honour the Bearer token to identify the user.
 *
 * Returns null if both fail (caller decides whether to block or allow degraded state).
 */
export async function exchangeWithFrappe(
  accessToken: string,
): Promise<FrappeUser | null> {
  const base =
    import.meta.env.VITE_API_BASE ?? 'https://everydaybankingsolutions.facilitix.app';

  try {
    // 1️⃣ Social login callback (preferred)
    const socialRes = await fetch(
      `${base}/api/method/frappe.integrations.oauth2_logins.custom/ebs`,
      {
        method: 'POST',
        headers: {
    ...
    if (socialRes.ok) {
      ...
      console.log('[Frappe] Social login exchange succeeded');
      return (json.message as FrappeUser) ?? null;
    }

    // 2️⃣ Fallback: get_logged_user
    console.warn(
      `[Frappe] Social login endpoint returned ${socialRes.status}, trying get_logged_user…`,
    );
    ...
    if (userRes.ok) {
      ...
      console.log('[Frappe] get_logged_user exchange succeeded');
      // Frappe returns just the email string in json.message for this endpoint
      return { email: json.message as string, full_name: json.message as string };
    }

    console.warn('[Frappe] Both token exchange attempts failed.');
    return null;
  } catch (err) {
    console.error('[Frappe] Token exchange error:', err);
    return null;
  }
}
```
...
// ── localStorage key for Frappe user ──────────────────────────────────────
const FRAPPE_USER_KEY = 'ebs_frappe_user';

// ── Types ──────────────────────────────────────────────────────────────────

interface AuthState {
  /** Resolved app user (IDP profile enriched with Frappe data) */
  user: AppUser | null;
  /** Raw Frappe user record — useful for roles, user_image, etc. */
  frappeUser: FrappeUser | null;
  /** IDP access_token — use as Bearer for Frappe API calls */
...
interface AuthContextType extends AuthState {
  ...
  /** Completes OIDC callback: code exchange → Frappe exchange → sets state */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [frappeUser, setFrappeUser]   = useState<FrappeUser | null>(null);
    ...
    (async () => {
      try {
        ...
        if (stored && !stored.expired && mounted) {
          // Restore Frappe user from localStorage
          const savedFrappe = localStorage.getItem(FRAPPE_USER_KEY);
          const frappe: FrappeUser | null = savedFrappe ? JSON.parse(savedFrappe) : null;

          setFrappeUser(frappe);
          setUser(buildAppUser(stored, frappe));
  ...
  const handleOidcCallback = useCallback(async (): Promise<AppUser | null> => {
    ...
    try {
      ...
      // 2️⃣ Exchange IDP access_token → Frappe user record
      //    Hits: https://everydaybankingsolutions.facilitix.app
      //    /api/method/frappe.integrations.oauth2_logins.custom/ebs
      const frappe = await exchangeWithFrappe(oidcUser.access_token);
      setFrappeUser(frappe);

      // Persist Frappe user to localStorage for session restore on next visit
      if (frappe) {
        localStorage.setItem(FRAPPE_USER_KEY, JSON.stringify(frappe));
      }

      // 3️⃣ Build final AppUser (Frappe data takes priority over IDP profile)
      const appUser = buildAppUser(oidcUser, frappe);
      setUser(appUser);
  ...
  const logout = useCallback(async () => {
    // Clear Frappe user from localStorage
  ...
  return (
    <AuthContext.Provider value={{
      user, frappeUser, accessToken, isLoading, error,
...
export default function Dashboard() {
  const { user, frappeUser, accessToken, logout } = useAuth();
    ...
    <div style={styles.root}>
      ...
      <div style={styles.container}>
        ...
        <div style={styles.grid}>
          ...
          <InfoCard
            label="Frappe Email"
            value={frappeUser?.email ?? '— (exchange pending)'}
            icon="🏢"
          />
          <InfoCard
            label="Frappe Full Name"
            value={frappeUser?.full_name ?? '— (exchange pending)'}
            icon="🪪"
          />
          {frappeUser?.roles && frappeUser.roles.length > 0 && (
            <InfoCard
              label="Frappe Roles"
              value={frappeUser.roles.join(', ')}
              icon="🛡️"
            />
          )}
          <InfoCard
